...
export class SignAndSendService {
  ...
  /**
   * Sign and send a message to the blockchain.
   */
  async signAndSendMessage(
    ...
    try {
      ...
      const extrinsic = this.api.message.send(message, metadata);
      ...
      const signature: string = await signTransaction(payload);
